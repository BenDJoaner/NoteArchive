...
struct SwipeCardView: View {
    @State var message: [SwipeCardMessage] = []
    @State var pageDatas: [DrawingPage] = []
    
    var body: some View {
        VStack {
            SwipeCarousel(items: message, id: \.id) { message, size in
                if let pageData = message.pageData as? Data, // 确保 pageData 是 Data 类型
...
//                    .clipShape(RoundedRectangle(cornerRadius: 20, style: .continuous))
        ...
        .onAppear {
            for index in 0..<pageDatas.count {
                message.append(SwipeCardMessage(pageData: pageDatas[index]))
            }
        }
    }
}
