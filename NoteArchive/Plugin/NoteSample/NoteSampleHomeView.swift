...
struct NoteSampleHomeView: View {
    @State private var searchText: String = ""
    @State private var selectedNote: NoteSampleNoteModel?
    @State private var deleteNote: NoteSampleNoteModel?
    ...
    var body: some View {
        NoteSampleSearchQueryView(searchText: searchText) { notes in
            ScrollView(.vertical) {
                VStack(spacing: 10) {
                    SearchBar()
    ...
    @ViewBuilder
    func CardView(_ note: NoteSampleNoteModel) -> some View {
        ZStack {
            if selectedNote?.id == note.id && animateView {
                ...
                RoundedRectangle(cornerRadius: 10)
                    ...
                    .overlay {
                        NoteSampleTitleNoteView(size: titleNoteSize, note: note)
                    }
    ...
    func createEmptyNote() {
        ...
        if let randomColor = colors.randomElement() {
            let NoteModel: NoteSampleNoteModel = .init(colorString: randomColor, title: "", content: "")
